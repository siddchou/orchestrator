...
REM Usage: run-step.bat <step-name>
...
if "%~1"=="" (
    echo Usage: run-step.bat ^<step-name^>
    exit /b 1
)
...
for /f "delims=" %%i in ('powershell -Command ^
    "$r=Invoke-RestMethod -Uri '%ORCHESTRATOR_URL%/api/steps/name/%STEP_NAME%/run' -Method Post -Headers @{Authorization='Bearer %JWT_TOKEN%'} -ContentType 'application/json'; $r.data.runId"') do set RUN_ID=%%i
...
:done
echo [run-step] Step '%STEP_NAME%' finished — Status: %RUN_STATUS%
